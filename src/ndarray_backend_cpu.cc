...
void ScalarMaximum(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [val](scalar_t x, scalar_t y) { return std::max(x, val); });
}
...
EWISE_UNARY_OP(EwiseLog, std::log)
EWISE_UNARY_OP(EwiseExp, std::exp)
EWISE_UNARY_OP(EwiseTanh, std::tanh)
void ScalarPower(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [val](scalar_t x, scalar_t y){return std::pow(x, val);});
}
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  /// BEGIN SOLUTION
  Fill(out, 0.0f);
  uint32_t SQ_TILE = TILE * TILE;
  for (uint32_t i = 0; i < m/TILE; ++i){
    for (uint32_t j = 0; j < p/TILE; ++j){
      for (uint32_t k = 0; k < n/TILE; ++k){
        AlignedDot(&a.ptr[i*n*TILE+k*SQ_TILE], &b.ptr[j*SQ_TILE+k*p*TILE], &out->ptr[i*p*TILE+j*SQ_TILE]);
      }
    }
  }
  /// END SOLUTION
