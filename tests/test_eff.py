...
def build():
    for file in ["matmul-eff.cu", "test_eff.cu"]:
        assert os.path.exists(file) and os.path.isfile(file), "CUDA files not exist"
    command = "nvcc -arch=sm_80  matmul-eff.cu test_eff.cu -o test_eff"
...
def main(M, N, K):
    command = f"./test_eff M {M} N {N} K {K} iters 200 stages 4"
    p = subprocess.run(
        command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
...
if __name__ == "__main__":
    ...
    with open("this_results.csv", "w") as fout:
        ...
        for cc in costs:
            ...
            print(f"{M},{N},{K},{'float16'},{'float32'},{runtime},{tflops}", file=fout)
    print("Done!")
