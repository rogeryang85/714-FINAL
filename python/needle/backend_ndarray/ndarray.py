...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        old_s = prod(self.shape)
        new_s = prod(new_shape)

        if old_s != new_s or not self.is_compact():
          # print(f"old: {old_s}; new: {new_s}; is_compact?: {self.is_compact()}")
          raise ValueError("new and old shapes not match")
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        # print(f"permute axes: {new_axes}", self.shape)
    ...
    def broadcast_to(self, new_shape):
        ...
        for i, dim in enumerate(self.shape):
          assert new_shape[i] == self.shape[i] or self.shape[i] == 1, \
            f"Cannot broadcast dimension {i} from {self.shape[i]} to {new_shape[i]}"
        new_strides = [0 if new_shape[i] != self.shape[i] and self.shape[i] == 1 else self.strides[i] for i, dim in enumerate(self.shape)]
    ...
    def __getitem__(self, idxs):
        ...
        for dim, s in enumerate(idxs):
            ...
            new_shape.append((stop - start + step - 1) // step)  # Effective size along this axis
            new_strides.append(self.strides[dim] * step)
            new_offset += start * self.strides[dim]

        # Return a new NDArray view with adjusted shape, strides, and offset
        return self.make(tuple(new_shape), strides=tuple(new_strides), device=self._device, handle=self._handle, offset=new_offset)
        ### END YOUR SOLUTION
    ...
    def ewise_or_scalar(self, other, ewise_func, scalar_func):
        ...
        out = NDArray.make(self.shape, device=self.device)
        if isinstance(other, NDArray):
            # print("ewise_or_scalar: ", self.device, other.device, out.device)
    ...
    def sum(self, axis=None, keepdims=False):
        # print("sum", self.shape, axis, keepdims)
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # If axes is None or empty, return the array as is
        ...
        # Create new strides and shape for the flipped array
        new_shape = list(self.shape)
        new_strides = list(self.strides)
        new_offset = 0
        
        # Calculate new offset and adjust strides for each axis to flip
        for axis in axes:
            if axis < 0:
                axis = len(new_shape) + axis
            new_offset += (new_shape[axis] - 1) * new_strides[axis]
            new_strides[axis] = -new_strides[axis]
        
        # Create new array with adjusted strides and offset
        ret = self.make(
            new_shape,
    ...
    def pad(self, axes):
        ...
        # Create slicing tuple to place original data
        slices = tuple(
            slice(axes[i][0], axes[i][0] + self.shape[i])
            for i in range(len(self.shape))
        )
        
        # Place original data in the padded array
        out[slices] = self
        return out
        ### END YOUR SOLUTION
...
def transpose(a, axes=None):
    """
    Transpose implementation that safely handles axes
    """
    ndim = len(a.shape)
    if axes is None:
        # Default case - swap last two dimensions
        if ndim <= 1:
            return a
        ...
        # Handle explicit axes
        if isinstance(axes, (tuple, list)) and len(axes) == 2:
            # Two axes specified - swap them
            new_axes = list(range(ndim))
            new_axes[axes[0]], new_axes[axes[1]] = new_axes[axes[1]], new_axes[axes[0]]
            return a.permute(tuple(new_axes))
        else:
            # Validate axes are within bounds
            axes = tuple(ax for ax in axes if ax < ndim)
